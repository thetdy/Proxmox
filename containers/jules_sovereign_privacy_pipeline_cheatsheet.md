...
_Run all JM commands from `/opt/joinmarket-clientserver/scripts/` using `torsocks`._

**Wallet Summary (View Balances & Addresses):**
torsocks python wallet-tool.py wallet.jmdat display

**View Recovery Seed:**
torsocks python wallet-tool.py wallet.jmdat showseed

**Extract Zpub (Mixdepth 0):**
torsocks python wallet-tool.py -m 0 wallet.jmdat
...
- **Cold Destination Config:** `/root/.walletwasabi/client/Wallets/JoinMarketHot.json`

---

## 6. Configuration Changes & States

- **Systemd Auto-Mounting:** Modified the `ExecStart` line in `wasabi.service` to auto-load both wallets into active memory on boot, while forcing JSON-RPC and Trace logging. The line now reads:
  `ExecStart=/opt/wasabi/WasabiWallet/wassabeed --loglevel=trace --jsonrpcserverenabled=true --wallet=Wasabi --wallet=JoinMarketHot`
- **Wallet State Syncing (Split-Brain Fix):** The headless daemon does not sync automatically with the desktop GUI. You must explicitly patch the server-side wallet configuration to prevent AllCoinsPrivate execution aborts in `/root/.walletwasabi/client/Wallets/JoinMarketHot.json` to `"AnonScoreTarget": 35` and `"PlebStopThreshold": "0.001"`.
- **Target Anonymity Score (`AnonScoreTarget`):** Changed from default (`5`/`10`) to `35` inside **both** `Wasabi.json` and `JoinMarketHot.json`. This ensures the sweep command loops internally until the score hits 35 before executing the final bridge transaction.
- **Economic Safeguard (`PlebStopThreshold`):** Temporarily lowered in `Wasabi.json` to `0.001` to allow a small `0.0045` BTC test UTXO to be picked up by the automated sweep engine. _(Note: Should be returned to `0.005` for mainnet operation)._
...
- **Trigger CoinJoin:**
  `curl -v --data-binary '{"jsonrpc":"2.0","id":"1","method":"startcoinjoin","params":["", true, true]}' http://127.0.0.1:37128/JoinMarketHot`
  _Note: Parameters map to `[ "Password", StopWhenAllMixed, OverridePlebStop ]`. A successful execution returns a blank body and an HTTP 200 OK._
- **Check Anonymity Sets:**
  `curl -s --data-binary '{"jsonrpc":"2.0","id":"1","method":"listcoins"}' http://127.0.0.1:37128/JoinMarketHot`
  _Note: Must use the V2 `listcoins` API. Deprecated V1 commands like `listunspentcoins` will fail silently._
...
- **Premature Sweep Execution:** Discovered that `startcoinjoinsweep` calculates its execution based on the _destination wallet's_ configuration file. If `JoinMarketHot.json` is set to an anonymity score of 10, Wasabi will sweep after a single round. Setting the destination file to 35 forces Wasabi to hold the funds in the hot wallet and mix internally until the threshold is met.
